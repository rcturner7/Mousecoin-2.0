...
unsigned int nStakeMinAge = 1.5 * 60 * 60;
...
const string strMessageMousecoin = "Mousecoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees, uint256 prevHash)
{
    ...
    {
        nSubsidy = COIN_POW_REWARD2;
    }
    else if(nBestHeight <= 100000)
    {
        nSubsidy = COIN_POW_REWARD3;
    }
    else if(nBestHeight <= 150000)
    {
        nSubsidy = COIN_POW_REWARD4;
    }
    else if(nBestHeight <= 40000)
    {
        nSubsidy = COIN_POW_REWARD5;
    }
    else
    {
        nSubsidy = COIN_POW_REWARD;
    }
...

{
    ...
    nSubsidy = nCoinAge * COIN_YEAR_REWARD2 * 33 / (365 * 33 + 8);
        ...
        nSubsidy = nCoinAge * COIN_YEAR_REWARD5 * 33 / (365 * 33 + 8);
    else if(nBestHeight <= 300000)
        nSubsidy = nCoinAge * COIN_YEAR_REWARD6 * 33 / (365 * 33 + 8);
    else if(nBestHeight <= 350000)
        nSubsidy = nCoinAge * COIN_YEAR_REWARD7 * 33 / (365 * 33 + 8);
    else if(nBestHeight <= 400000)
        nSubsidy = nCoinAge * COIN_YEAR_REWARD8 * 33 / (365 * 33 + 8);
    else if(nBestHeight <= 550000)
        nSubsidy = nCoinAge * COIN_YEAR_REWARD9 * 33 / (365 * 33 + 8);
    else if(nBestHeight <= 600000)
        nSubsidy = nCoinAge * COIN_YEAR_REWARD10 * 33 / (365 * 33 + 8);
    else if(nBestHeight <= 650000)
        nSubsidy = nCoinAge * COIN_YEAR_REWARD11 * 33 / (365 * 33 + 8);
    else if(nBestHeight <= 700000)
        nSubsidy = nCoinAge * COIN_YEAR_REWARD12 * 33 / (365 * 33 + 8);
    else if(nBestHeight <= 750000)
        nSubsidy = nCoinAge * COIN_YEAR_REWARD13 * 33 / (365 * 33 + 8);
    else
        nSubsidy = nCoinAge * COIN_YEAR_REWARD14 * 33 / (365 * 33 + 8);
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // MainNet:
        const char* pszTimestamp = "RIP mouse 3 mic3 2.0 take over on Mar 14th, 2022";
        CTransaction txNew;
        txNew.nTime = 1647016158;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1647016158;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 0 : 1200561;
	...
	fflush(NULL);
        ...
        assert(block.hashMerkleRoot == uint256("0x8e4025926ebe67be8af3bdda5910ccc8efc28871464d70d0cbb52fe0b1ce2b3f"));
